		...
		if response.status!=404:
			...
			if Major!= None and Major!="Not found":
				...
				for course in response.xpath('//div[@class="course-table"]/div'):
					...
					# Instructor=course.xpat h('div[2]/table/tr[2]/td[7]/a/text()').extract_first()
					syllabus=course.xpath('div[2]/table/tr[2]/td[9]/a/@href').extract_first()
					try:
						if syllabus!=None:
							if syllabus.endswith('.pdf') or syllabus.endswith('.doc'):
								self.logger.info(syllabus)
								yield Request(syllabus, callback=self.save_pdf)
					except IOError:
						print("Error: none")
